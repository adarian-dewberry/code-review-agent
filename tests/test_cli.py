...
def mock_review_result():
    ...
    return ReviewResult(
        security=ReviewCategory(category="Security Analysis", issues=[]),
        logic=ReviewCategory(category="Logic Issues", issues=[]),
...
def test_main_with_file_path(tmp_path, mock_review_result):
    ...
    with patch("code_review_agent.cli.CodeReviewAgent") as MockAgent:
        ...
        assert (
            "APPROVE" in mock_stdout.getvalue()
            or "Security Analysis" in mock_stdout.getvalue()
        )
...
def test_main_file_not_found():
    ...
    Note: Error message is intentionally generic to prevent information disclosure (F-002 fix).
    """
    with patch("sys.argv", ["code-review", "review", "nonexistent.py"]):
        with patch("sys.stderr", new_callable=StringIO) as mock_stderr:
            ...
            # Check for generic message (security fix F-002: no path disclosure)
            assert "could not read input file" in mock_stderr.getvalue().lower()
...
def test_main_json_format(tmp_path, mock_review_result):
    ...
    with patch("code_review_agent.cli.CodeReviewAgent") as MockAgent:
        ...
        with patch(
            "sys.argv", ["code-review", "review", "--format", "json", str(test_file)]
        ):
            with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
                main()
...
def test_main_custom_config(tmp_path, mock_review_result):
    ...
    with patch("code_review_agent.cli.CodeReviewAgent") as MockAgent:
        ...
        with patch(
            "sys.argv",
            ["code-review", "review", "--config", str(config_file), str(test_file)],
        ):
            with patch("sys.stdout", new_callable=StringIO):
                main()
