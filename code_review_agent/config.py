...
class ReviewConfig(BaseModel):
    ...
    enabled_categories: list[str] = Field(default_factory=lambda: ["security", "logic", "performance", "compliance"])
...
class Config(BaseModel):
    ...
    def validate_api_key(self) -> None:
        ...
        if not self.anthropic_api_key:
            raise ValueError(
                "Anthropic API key not configured. Set ANTHROPIC_API_KEY " "environment variable or add to config.yaml"
            )
