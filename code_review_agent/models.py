...
class ReviewResult(BaseModel):
    ...
    def to_markdown(self) -> str:
        ...
        if self.file_path:
            lines.extend([f"**File:** `{self.file_path}`", ""])
        ...
        if self.summary.top_issues:
            ...
            for top_issue in self.summary.top_issues:
                lines.append(f"- {top_issue}")
            lines.append("")

        # Detailed findings
        for category in [self.security, self.logic, self.performance, self.compliance]:
            if category.issues:
                lines.extend([f"## {category.category.title()} Review", ""])
        ...
        if self.sdl_metadata:
            lines.extend(["---", "", "# SDL Multi-Agent Security Squad Analysis", ""])
            ...
            if "sdl_status" in self.sdl_metadata:
                ...
                if "gate_checks" in sdl_status:
                    lines.extend(["### Phase Gate Checklist", ""])
                ...
                if "champion_duties" in sdl_status:
                    ...
                    lines.extend(["### Security Champion Checklist", ""])
